...
con.connect(function (err) {
  ...
  const folderPath = path.join(__dirname, '..', 'server', 'views', 'image', 'neutres', 'general');
              ...
              try {
                ...
                for (let i = 0; i < files.length; i++) {
                  ...
                  await new Promise((resolve, reject) => {
                    con.query("INSERT INTO Images (ImageSetID, FilePath, Question) SELECT ImageSets.ID, ?, ? FROM ImageSets WHERE ImageSets.URLUsage = ?", [path.basename(filePath), '', setName], function (err, result) {
                      if (err) {
                        reject(err);
                        return;
                      }
